...
import type { FixtureRecord, FixtureStore, RoundRecord, SyncLogger } from "./types";
...
export async function syncActiveRoundFixtures(options: {
  ...
  const fixtures = await store.getFixturesByIds(fixtureIds);
  const fixturesById = new Map(fixtures.map((fixture) => [fixture.id, fixture]));
  const fixturesByApiId = new Map(fixtures.map((fixture) => [fixture.apiId, fixture]));
  ...
  logger.info(`Updated ${updated} fixtures`);
  return updated;
}
